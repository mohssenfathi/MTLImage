...
vertex TextureMappingVertex transform_vertex(unsigned int vertex_id [[ vertex_id ]],
                                             constant TransformUniforms &uniforms [[ buffer(0) ]]) {
    
    float4x4 position = float4x4(float4( -1.0, -1.0, 0.0, 1.0 ),
                                 float4(  1.0, -1.0, 0.0, 1.0 ),
                                 float4( -1.0,  1.0, 0.0, 1.0 ),
                                 float4(  1.0,  1.0, 0.0, 1.0 ));
    
    float4x2 coordinates = float4x2(float2( 0.0, 1.0 ),
                                    float2( 1.0, 1.0 ),
                                    float2( 0.0, 0.0 ),
                                    float2( 1.0, 0.0 ));

    float4x4 orthographicMatrix = float4x4(float4( 1.0, 0.0, 0.0, 0.0 ),
                                           float4( 0.0, 1.0, 0.0, 0.0 ),
                                           float4( 0.0, 0.0, 1.0, 0.0 ),
                                           float4( 0.0, 0.0, 0.0, 1.0 ));
    
//    float4 pos = position[vertex_id];
    float4 pos = uniforms.transform * position[vertex_id] * orthographicMatrix;
    
    TextureMappingVertex outVertex;
    outVertex.position = pos;
